...
from preprocessing.flightphase import fuzzylabels
# In-House functions
from cluster.resize_flights import resize, resize_norm, resize_4cluster, resize_norm_4cluster
from cluster.distances_matrix import distances
import os
########################################################################################
"""Airport coordinates"""
########################################################################################
...
def cluster_all(departure,arrival):
    print('++++++++========This route is:', departure,arrival)
    ########################################################################################
    """Inputs"""
    ########################################################################################
    ...
    # CSV flights import    
    name = "/Users/aarc8/Documents/github/Dataprocess/Database/Data4Clustering/"+departure+ "/Data4Clustering_"+departure+arrival+".csv"
    ...
    def save_traj(traj,departure,arrival):

        traj = np.around(traj,decimals=4)
        flight = pd.DataFrame({'times':traj[:, 0],'lon':traj[:, 1],'lat':traj[:, 2],'alt':traj[:, 3],'speed':traj[:, 4],'roc':traj[:, 5],'tas':traj[:, 6],'mach':traj[:, 7]})
        
        times = np.array(flight['times'])
        alts = np.array(flight['alt'])
        spds = np.array(flight['speed'])
        rocs = np.array(flight['roc'])

        labels = fuzzylabels(times, alts, spds, rocs)

        flight['flight_phase'] = labels

        alt_cruz = flight[['alt','flight_phase']]
        alt_cruz = alt_cruz.query('flight_phase == "CR"')
        max_altitude = alt_cruz['alt'].max()
        max_altitude = round(max_altitude)


        path_cluster = "Database/Routes/"+departure+"-"+arrival+"/"
        # Check whether the specified path exists or not
        path_cluster_isExist = os.path.exists(path_cluster)

        if not path_cluster_isExist:
        ...
        flight.to_csv(path_cluster+str(max_altitude) + '.csv')
    ...
    for i in range(num_Hclusters):
        ...
        kmedoids= KMedoids(n_clusters = num_Vclusters[i], init ='k-medoids++', random_state = 0)
        kmedoids.fit_predict(Ds[i])

        print(kmedoids.medoid_indices_[0])

        
        # print(vclusters[0][kmedoids.medoid_indices_[0]])

        for k in range(len(kmedoids.medoid_indices_)):
            save_traj(vclusters[i][kmedoids.medoid_indices_[k]],departure,arrival)
    ...
    def plot_cluster(traj_lst, cluster_lst):
        ...
        for traj, cluster in zip(traj_lst, cluster_lst):
            ...
            if cluster == -1:
                ...
                plt.plot(traj[:, 1], traj[:, 2], c=color_lst[cluster % len(color_lst)],linewidth=1)
    ...
    def plot_cluster3D(traj_lst, cluster_lst):
        ...
        for traj, cluster in zip(traj_lst, cluster_lst):
            ...
            if cluster == -1:
                ...
                ax.plot(traj[:, 1], traj[:, 2], traj[:, 3],c=color_lst[cluster % len(color_lst)],linewidth=1)
