...
# alt_lohi = fuzz.gaussmf(alt_range, 25000,10000)
# alt_hi = fuzz.gaussmf(alt_range, 40000,10000)


# # Mexer aqui para melhorar cruseiro
# roc_zero = fuzz.gaussmf(roc_range, 0, 50)
# roc_plus = fuzz.smf(roc_range, 10, 1000)
# roc_minus = fuzz.zmf(roc_range, -1000, -10)

# spd_lo = fuzz.gaussmf(spd_range, 0, 50)
# spd_md = fuzz.gaussmf(spd_range, 300, 300)
# spd_hi = fuzz.gaussmf(spd_range, 600, 50)
...
# alt_gnd = fuzz.zmf(alt_range, 0, 200)
# alt_lo = fuzz.gaussmf(alt_range, 10000, 5000)
# alt_hi = fuzz.gaussmf(alt_range, 20000, 10000)
# alt_cru = fuzz.smf(alt_range,30000,40000)

# roc_zero = fuzz.gaussmf(roc_range, 0, 200)
# roc_plus = fuzz.smf(roc_range, 10, 1000)
# roc_minus = fuzz.zmf(roc_range, -1000, -200)
# roc_cru = fuzz.gbellmf(roc_range,250,10,0)
...
roc_plus = fuzz.smf(roc_range, 10, 1000)
roc_minus = fuzz.zmf(roc_range, -1000,-10)
roc_cru = fuzz.pimf(roc_range,-400,-10,10,500)

spd_lo = fuzz.gaussmf(spd_range, 0, 50)
spd_md = fuzz.gaussmf(spd_range, 200, 100)
spd_hi = fuzz.gaussmf(spd_range, 500, 100)
...
def plot_logics():
    ...
    plt.plot(alt_range, alt_lohi, lw=2, label='LowHigh')
...
def fuzzylabels(ts, alts, spds, rocs, twindow=60):
    ...
    # alts = sp.signal.savgol_filter(alts, 11, 2)
    spds = UnivariateSpline(ts, spds)(ts)
    # spds = sp.signal.savgol_filter(spds, 11, 2)
    rocs = UnivariateSpline(ts, rocs)(ts)
    # rocs= sp.signal.savgol_filter(rocs, 11, 2)
    ...
    for tw in range(0, int(max(twindows))):
        ...
        # alt_level_lohi = fuzz.interp_membership(alt_range, alt_lohi, alt)
        alt_level_hi = fuzz.interp_membership(alt_range, alt_hi, alt)
        alt_level_cru = fuzz.interp_membership(alt_range, alt_cru, alt)
        ...
        roc_level_cru = fuzz.interp_membership(roc_range, roc_cru, roc)
        ...
        # rule_level = min(alt_level_lo,alt_level_lohi, roc_level_zero, spd_level_md)
